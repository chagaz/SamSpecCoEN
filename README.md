...
-----------------------------
Reference healthy data (E-MTAB-62) must be pre-processed to be stored in a data structure similar to that of the ACES data. This is done by running
```python preprocess-MTAB-62.py```
...
```python CoExpressionNetwork.py DMFS ../ArrayExpress/postproc ../outputs/U133A_combat_DMFS```
creates sample-specific coexpression networks for the entire dataset. The network structure (list of edges) is stored under `outputs/U122A_combat_DMFS/edges.gz` and its weights under `outputs/U122A_combat_DMFS/lioness/edges_weights.gz` (for the LIONESS approach) and `outputs/U122A_combat_DMFS/reglref_edges_weights.gz` (for the regression line approach).
...
creates 5 folds for cross-validation. The network structure (list of edges) is stored under `outputs/U122A_combat_DMFS/edges.gz`. The network weights for the REGLREF approach are stored under
`outputs/U122A_combat_DMFS/reglref_edges_weights.gz`.

Network weights are sample-dependent for LIONESS. For each fold from k=0 to k=4, they are stored under `outputs/U122A_combat_DMFS/<k>/lioness/edges_weights.gz` (for the LIONESS approach).
...
refc_dir=data/SamSpecCoEN/ArrayExpress # pre-processed with preprocess-MTAB-62.py

# Create repeat/fold indices
for repeat in {0..9}
do
    python setupSubtypeStratifiedCV_writeIndices.py data/SamSpecCoEN ${repeat}
done

# Create LIONESS networks (fold-specific because they depend on the samples)
for repeat in {0..9}
do
    for fold in {0..9}
    do
	    python setupSubtypeStratifiedCV_computeNetworks.py ${aces_dir} ${data_dir} ${fold} ${repeat}
    done
done

# Create REGLREF networks
python CoExpressionNetwork.py RFS ../ArrayExpress/postproc ../outputs/U133A_combat_RFS
...
do
    for fold in {0..9}
    do
	   python setupSampledLOSOCV_computeNetworks.py ${aces_dir} ${data_dir} ${fold} ${repeat}
    done
 done
...
for repeat in {0..9}
do
    for network in lioness reglref
    do
...
Lukk, M., Kapushesky, M., Nikkilä, J., Parkinson, H., Goncalves, A., Huber, W., ... & Brazma, A. (2010). A global map of human gene expression. Nature biotechnology, 28(4), 322-324.
